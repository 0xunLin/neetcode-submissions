# This solution is the simplest one-line code, with O(N) time complexity and O(N) space complexity, where the set(nums) creates a pointer from the stack frame to the heap where an element is hashed to put into a hashed heap bucket according to its own hash, so duplicate elements whose values have already been put into the heap are skipped on check, if fresh new lement, then it passes the check and is put into the hashed bucket location.
# In this solution we use the O(1) len() function to find the size(length) of the original nums array, and its no duplicate hash set, we compare these two, if not equal -> False thus contains duplicates and, if equal -> True thus no duplicates
# class Solution:
